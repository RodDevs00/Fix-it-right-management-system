             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM transaction_table JOIN employee on encoder = employee.employee_fname WHERE transaction_sched >= '$chosendate' AND transaction_sched <= '$chosendate2' ORDER BY transaction_sched ASC";
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM parts_table WHERE item_transaction_sched >= '$chosendate' AND item_transaction_sched <= '$chosendate2' AND item_status !='Pending' ORDER BY item_transaction_sched ASC";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     if(mysqli_num_rows($resultinventory)>0){
                         while ($row = mysqli_fetch_array($resultinventory)) {
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT * FROM expense_table WHERE expense_date >= '$chosendate' AND expense_date <= '$chosendate2' ORDER BY expense_date ASC";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     if(mysqli_num_rows($resultinventory)>0){
                        while ($row = mysqli_fetch_array($resultinventory)) {
                 ...
                 <?php
                            $totalsql = "SELECT SUM(expense_price) as totals FROM expense_table WHERE expense_date >= '$chosendate' AND expense_date <= '$chosendate2';";
                            $totalresult= mysqli_query($con,$totalsql);
                            $totaldata = mysqli_fetch_assoc($totalresult);
                            //echo $data['total'];
                            $totalsum = $totaldata['totals'];
                            $totallabor = "SELECT SUM(price) as totalpricelabor FROM transaction_table WHERE transaction_sched >= '$chosendate' AND transaction_sched <= '$chosendate2';";
                            ...
                            $totalparts = "SELECT SUM(item_total) as totalpriceprart FROM parts_table  WHERE item_transaction_sched >= '$chosendate' AND item_transaction_sched <= '$chosendate2' AND item_status !='Pending';";
