             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT COUNT(name) as countamount,name,SUM(price) as price FROM transaction_table  WHERE MONTH(`transaction_sched`)=MONTH( CURRENT_DATE ) GROUP BY name ORDER BY price";
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT SUM(item_total) as item_total, SUM(item_amount) as item_amount,item_name FROM parts_table WHERE MONTH(`item_transaction_sched`)=MONTH( CURRENT_DATE ) GROUP BY item_name";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT SUM(expense_price) as expense_price,expense_name,COUNT(expense_id) as expense_amount FROM expense_table WHERE MONTH(`expense_date`)=MONTH( CURRENT_DATE ) GROUP by expense_name ORDER by expense_price";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
                ...
                <table border="1" width="100%" > 
                 ...
                 <?php
                            ...
                            $totallabor = "SELECT SUM(price) as totalpricelabor FROM transaction_table WHERE MONTH(`transaction_sched`)=MONTH( CURRENT_DATE )";
                            ...
                            $totalparts = "SELECT SUM(item_total) as totalpriceprart FROM parts_table WHERE MONTH(`item_transaction_sched`)=MONTH( CURRENT_DATE ) AND item_status !='Pending';";
