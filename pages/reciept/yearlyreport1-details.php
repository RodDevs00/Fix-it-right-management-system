                ...
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     echo '<td>'.$prodmount.'</td>';
                      ...
                      echo '<td>'.$row['transaction_sched'].'</td>';
                      echo '</tr>';
                      }
                      ?>
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT SUM(item_total) as item_total, SUM(item_amount) as item_amount,item_name,item_transaction_sched FROM parts_table WHERE YEAR(`item_transaction_sched`)>= '$test' AND YEAR(`item_transaction_sched`) <= '$test2' AND item_status !='Pending' GROUP BY item_name";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
                     ...
                     echo '<td>'.$row['item_transaction_sched'].'</td>';
             ...
             <tbody>
                <?php 
                    $currentdate = date("Y-m-d");
                     ...
                     $sqlinventory = "SELECT expense_date,SUM(expense_price) as expense_price,expense_name,COUNT(expense_id) as expense_amount FROM expense_table WHERE YEAR(`expense_date`)>= '$test' AND YEAR(`expense_date`) <= '$test2' GROUP by expense_name ORDER by expense_price";
                     $resultinventory = mysqli_query($con, $sqlinventory) or die(mysqli_error($db));
                     while ($row = mysqli_fetch_array($resultinventory)) {
                ...
                <table border="1" width="100%" > 
                 ...
                 <?php
                            $totalsql = "SELECT SUM(expense_price) as totals FROM expense_table WHERE YEAR(`expense_date`)>= '$test' AND YEAR(`expense_date`) <= '$test2'";
                            $totalresult= mysqli_query($con,$totalsql);
                            $totaldata = mysqli_fetch_assoc($totalresult);
                            //echo $data['total'];
                            $totalsum = $totaldata['totals'];
                            $totallabor = "SELECT SUM(price) as totalpricelabor FROM transaction_table WHERE YEAR(`transaction_sched`)>= '$test' AND YEAR(`transaction_sched`) <= '$test2'";
                            ...
                            $totalparts = "SELECT SUM(item_total) as totalpriceprart FROM parts_table WHERE YEAR(`item_transaction_sched`)>= '$test' AND YEAR(`item_transaction_sched`) <= '$test2' AND item_status !='Pending';";
